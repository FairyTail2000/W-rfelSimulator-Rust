			...
			if no_dice_select {
				...
				match io::stdin().read_line(&mut input) {
					Ok(_n) => {
						...
						finished = handle_input(removed, old, &colored_dice, &normal_dices, error_message.as_str());
					}
				...
				finished = handle_input(input.to_string(), old, &colored_dice, &normal_dices, error_message.as_str());
			}
		}
	}
	Ok(())
